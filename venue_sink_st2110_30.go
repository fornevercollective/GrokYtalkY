...
// writeST2110MultiEssenceSDP combines 2110-20 video + 2110-30 audio in one SDP session.
func writeST2110MultiEssenceSDP(path, vHost string, vPort int, aHost string, aPort, w, h, fps int, audio ST211030Opts, sync SyncClockReport) error {
	...
	now := time.Now().Unix()
	vFmtp := fmt.Sprintf(
		"sampling=YCbCr-4:2:2; width=%d; height=%d; exactframerate=%d; depth=8; colorimetry=BT709; PM=2110GPM; SSN=ST2110-20:2017; TP=2110TPN; interlace=0",
		w, h, fps,
	)
...
i=ST 2110-20 video + ST 2110-30 audio. System timing ST 2110-10 / PTP %s. Compliant=%v.
c=IN IP4 %s/32
t=0 0
a=tool:GrokYtalkY/%s
a=type:broadcast
...
a=ptime:%.3f
a=recvonly
`, now, now, vHost, PTPProfileST2059, sync.Compliant, vHost, Version, tsRef,
		vPort, vFmtp, w, h, fps,
